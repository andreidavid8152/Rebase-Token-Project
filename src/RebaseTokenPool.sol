...
contract RebaseTokenPool is TokenPool {
    ...
    function lockOrBurn(Pool.LockOrBurnInV1 calldata lockOrBurnIn) external returns (Pool.LockOrBurnOutV1 memory lockOrBurnOut) {
        ...
        IRebaseToken(address(i_token)).burn(address(this), lockOrBurnIn.amount); // Burn occurs inside the pool, because the sender sends the tokens to the pool for custody
